...
In addition to exploring the projected population size based on the current demographic rates, we can also explore scenarios with higher or lower values for the demographic rates to understand how potential future changes could impact the population. To demonstrate this click the "Add alternative scenario" button, fill in a descriptive name for the scenario, (e.g "Increase survival"), and move the survival slider to 90%, then click Run model. This will add the results for the alternate scenario to the graphs and tables in a different colour. We can see that with 90% survival the population is more likely to be increasing over time but there are still some plausible trajectories with a decreasing population. The table shows that in the "Increase survival" scenario we have increased female survival by 7% of the survival in the current scenario (i.e. $(90 - 84)/84 \times 100 = 7$)
...
To update the data used in the app, follow these steps: 1. Update the data in your Google spreadsheet. 2. Click on the **Update Data** button and if necessary follow the instructions to allow the app access to your Google Drive account. 4. Wait while the model is re-fit, this could take \~ 5 mins. 5. The app will automatically update the demographic rates and projections based on the new data.
