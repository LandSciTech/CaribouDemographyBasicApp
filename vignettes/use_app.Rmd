...
<!-- Not using because I think this is all covered in the text but a useful method! -->
<!-- ## Key terms -->
...
<!-- **Note the background in the images doesn't use the right ggplot theme** -->
...
The app utilizes the [`bboutools`](https://poissonconsulting.github.io/bboutools/articles/bboutools.html) package to fit Bayesian models to caribou demographic data [@dalgarno2025]. `bboutools` fits Bayesian models of annual recruitment and survival using generalized linear mixed-effects models where year is treated as a random effect if there is data from more than 5 years. The models are summarized for the recruitment and survival rates by taking samples from the posterior distribution and taking the mean to estimate the rate in a typical year, the standard deviation to estimate the variation in the rates and the 95% Credible Interval to estimate the upper and lower bounds of plausible values for the rate, given the survey data provided. These models are only run when the data is updated and the resulting rates are stored in the app and used to make projections.

**How is interannual variation done**

**Should I explain the sharing data among years extension to bboutools?**
...
**How are the "beta" and "logistic" methods in doSim the same?**

**Should we be documenting the beta type in doSim?**

## Uncertainty

**This might be covered enough already**
