---
title: "Utilisation de l’application v1"
output: 
  bookdown::html_document2:
      base_format: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Utilisation de l’application v1}
...
```{=html}
<!---
NOTE this needs to be updated with new changes made in the english version
...
Cet onglet comprend également un glossaire des termes utilisés ainsi qu'une foire aux questions.
...
Seuls les utilisateurs ayant installé le package R via la console, plutôt que la version de l’application fournie, peuvent modifier les données incluses dans l’application.
...
1.  Créez une feuille de calcul Google pour vos données d’enquête. La feuille doit respecter le format utilisé par [bboutools](https://poissonconsulting.github.io/bboutools/articles/bboutools.html#providing-data). Vous pouvez copier ce [modèle Google Sheet](https://docs.google.com/spreadsheets/d/1i53nQrJXgrq3B6jO0ATHhSIbibtLq5TmmFL-PxGQNm8/edit?usp=sharing) et le remplir avec vos propres données. Notez que les onglets « population_estimates » et « data_description » ne sont pas inclus dans l'exemple bboutools. L'onglet « population_estimates » fournit une estimation de la taille de la population féminine qui sera utilisée comme point de départ pour les simulations. Pour chaque population, indiquez une limite inférieure et supérieure de la population féminine estimée. Si plusieurs années d'estimations de population sont fournies, seule la plus récente sera utilisée. L'onglet « data_description » fournit un espace pour décrire les données qui sont rapportées dans les autres feuilles en plusieurs langues. Vous pouvez formater le texte de la description à l'aide du langage Markdown. Vous trouverez [ici](https://www.markdownguide.org/cheat-sheet/) une fiche pratique sur ce format.
